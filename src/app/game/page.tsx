...
import { addDoc, and, collection, db, doc, getDocs, limit, or, query, serverTimestamp, updateDoc, where } from "@/utils/firebase";
...
function Game() {
  ...
  const findGame = async () => {
    const q = query(collection(db, 'games'), and(or(where("black", "==", ""), where("white", "==", "")), where("minutes", "==", min), where("addSeconds", "==", sec)), limit(1));
    ...
    <div className="flex flex-col w-full h-full items-center justify-center">
      ...
      <div className="flex gap-2 mt-5">
        <div className="flex flex-col">
          <button disabled={!name} className="px-10 py-4 text-gray-200 bg-blue-400 rounded hover:bg-blue-800" onClick={createGame}>Create Game</button>
          <div className="text-xs text-gray-200 py-1 px-4">You already have an opponent you wanna play with</div>
        </div>
        <div className="flex flex-col">
        <button disabled={!name} className="px-10 py-4 text-gray-200 bg-red-400 rounded hover:bg-red-800" onClick={findGame}>Find Game</button>
        <div className="text-xs text-gray-200 py-1 px-4">You'd like to be randomly matched with an opponent</div>
        </div>
      </div>
