...
function WaitingOnPlayer({ movesTime, name, startGame }: Props) {
  ...
  return (
    <div className="flex flex-col items-center justify-center absolute h-full w-full z-50 opacity-90 bg-black left-0 top-0 text-center">
      ...
      <div className="text-gray-200 mt-4">Share link to invite an opponent or invite watchers. <button onClick={() => copyLink()} className="bg-green-700 p-2 rounded hover:bg-green-950">{buttonText}</button></div>
    </div>
  );
}
